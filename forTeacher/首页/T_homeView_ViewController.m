...
#import "T_homeView_ViewController.h"
#import "H_login_ViewController.h"
#import "WorkDetail_ViewController.h"
#import "passDelegate.h"
//#import "RemarkWork_ViewController.h"

@interface T_homeView_ViewController ()<UITableViewDataSource,UITableViewDelegate,loginDelegate,UIViewPassValueDelegate>
{
...
@implementation T_homeView_ViewController
...
- (void)viewDidLoad {
    ...
    H_login_ViewController * login = [[H_login_ViewController alloc] init];
    login.delegate = self;
...
//    NSLog(@"id = %@",[[NSUserDefaults standardUserDefaults] objectForKey:@"user_id"] );
    ...
    [manager GET:[NSString stringWithFormat:@"http://114.215.125.31/api/v1/teachers/%@/work_papers",[[NSUserDefaults standardUserDefaults]objectForKey:@"user_id"]]parameters:parameters success:^(AFHTTPRequestOperation *operation, id responseObject){
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    WorkDetail_ViewController * workDetail = [[WorkDetail_ViewController alloc] init];
    workDetail.workDic = [workArr objectAtIndex:indexPath.section];
    workDetail.delegate = self;
    [self.navigationController pushViewController:workDetail
                                         animated:YES];
}

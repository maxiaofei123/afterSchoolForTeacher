...
@interface releaseWork_ViewController ()<UIScrollViewDelegate,UIActionSheetDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,AVAudioSessionDelegate,AVAudioPlayerDelegate,UITextViewDelegate,UITableViewDataSource,UITableViewDelegate>
{
...
@implementation releaseWork_ViewController
...
- (void)viewDidLoad {
    ...
    UIBarButtonItem *anotherButton = [[UIBarButtonItem alloc] initWithTitle:@"提交  " style:UIBarButtonItemStylePlain target:self action:@selector(commitHomeWorkRelease:)];
...
-(void)drawtextView
{
    ...
    //作业类型
    chooseTypeView = [[UIView alloc] initWithFrame:CGRectMake(0,320+70,Main_Screen_Width-20, 50)];
...
-(void)releaseView
{
    ...
    int h = 80;
    NSLog(@"count =%d",imageMutableArr.count);
    if (imageMutableArr.count>0) {
        ...
        for (int i=0; i<imageMutableArr.count; i++) {
            ...
            bt.tag = i ;
            [bt addTarget:self action:@selector(deletePhotos:) forControlEvents:UIControlEventTouchUpInside];
    ...
    if (vedioMutableArr.count>0) {
        ...
        vedioScrollView.contentSize = CGSizeMake(108 * imageMutableArr.count+10,100);
        ...
        for (int i=0; i<vedioMutableArr.count; i++) {
            ...
            [bt setImage:[UIImage imageNamed:@"delete.png"] forState:UIControlStateNormal];
            bt.tag = i ;
            [bt addTarget:self action:@selector(deleteVedios:) forControlEvents:UIControlEventTouchUpInside];
    ...
    if (320+h+50 > Main_Screen_Height-64-59) {
        _releaseScrollView.contentSize = CGSizeMake(Main_Screen_Width-20,320+h+60);
    }
}
...
-(void)commitHomeWorkRelease:(UIButton *)sender
{
    ...
    UIAlertView * alert = [[UIAlertView alloc] initWithTitle:@"提示" message:@"是否要提交作业!" delegate:self cancelButtonTitle:@"提交" otherButtonTitles:@"取消", nil];
    alert.tag = 1001;
    [alert show];
}
...
-(void)commitHomeWork
{
    ...
    if (str1.length >0 && str2.length > 0&&classStr.length>0) {
        ...
        [manger POST:[NSString stringWithFormat:@"http://114.215.125.31/api/v1/teachers/%@/work_papers",[[NSUserDefaults standardUserDefaults]objectForKey:@"user_id"]] parameters:dic success:^(AFHTTPRequestOperation *operation, id responseObject) {
...
-(void)creatMedia:(NSString *)workId
{
    ...
    if (count>0) {
        ...
        for (int i =0; i<mp3MutableArr.count; i++) {
            AFHTTPRequestOperationManager * manager = [AFHTTPRequestOperationManager manager];
            [manager POST:[NSString stringWithFormat: @"http://114.215.125.31/api/v1/work_papers/%@/media_resources",workId] parameters:nil constructingBodyWithBlock:^(id<AFMultipartFormData> formData){
            ...
            } success:^(AFHTTPRequestOperation *operation, id responseObject)
             {
                 ...
                 [mp3fileManager removeItemAtPath:[mp3MutableArr objectAtIndex:i] error:nil];
        ...
        for (int i =0; i<vedioMutableArr.count; i++) {
            ...
            [manager POST:[NSString stringWithFormat: @"http://114.215.125.31/api/v1/work_papers/%@/media_resources",workId] parameters:nil constructingBodyWithBlock:^(id<AFMultipartFormData> formData){
                ...
                NSData * mp4data = [NSData dataWithContentsOfFile:[vedioMutableArr objectAtIndex:0]];
...
-(void)playMusic:(UIButton *)sender
{
    ...
    }else if (sender.tag ==3)
    {
        ...
        _streamer = nil;
        ++ index ;
        if (index>=mp3MutableArr.count) {
            index = mp3MutableArr.count-1;
        }
    }
    
    if (!_streamer) {
        self.streamer = [[AudioStreamer alloc] initWithURL:[NSURL URLWithString:[mp3MutableArr objectAtIndex:index]]];
           }
    if (![_streamer isPlaying]) {
        [_streamer start];
        [playMp3 setImage:[UIImage imageNamed:@"playMp3.png"] forState:UIControlStateNormal];
        ...
        [_streamer pause];
        [playMp3 setImage:[UIImage imageNamed:@"pause.png"] forState:UIControlStateNormal];
    }
}
...
- (void)paperWorkPlaybackStateChanged:(NSNotification *)notification
{
    ...
    {
        [playMp3 setImage:[UIImage imageNamed:@"playMp2.png"] forState:UIControlStateNormal];
...
#pragma mark -choosePhotosAndVideo
...
-(void)deleteVedios:(UIButton *)sender
{
